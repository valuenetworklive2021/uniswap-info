...
const DashGrid = styled.div`
  ...
  grid-template-areas: 'number name valueswap return';
  ...
  @media screen and (max-width: 740px) {
    ...
    grid-template-areas: 'name valueswap return';
  }

  @media screen and (max-width: 500px) {
    grid-template-columns: 2.5fr 1fr;
    grid-template-areas: 'name valueswap';
  }
`
...
const SORT_FIELD = {
  VALUE: 'VALUE',
  VALUESWAP_RETURN: 'VALUESWAP_RETURN',
}
  ...
  const positionsSorted =
    ...
    positions
      ...
      .sort((p0, p1) => {
        ...
        if (sortedColumn === SORT_FIELD.VALUESWAP_RETURN) {
          return p0?.valueswap?.return > p1?.valueswap?.return ? (sortDirection ? -1 : 1) : sortDirection ? 1 : -1
        }
        ...
        {!below500 && (
          <Flex alignItems="center" justifyContent="flexEnd">
            <ClickableText
              area="return"
              onClick={() => {
                setSortedColumn(SORT_FIELD.VALUESWAP_RETURN)
                setSortDirection(sortedColumn !== SORT_FIELD.VALUESWAP_RETURN ? true : !sortDirection)
              }}
            >
              {below740 ? 'Fees' : 'Total Fees Earned'}{' '}
              {sortedColumn === SORT_FIELD.VALUESWAP_RETURN ? (!sortDirection ? '↑' : '↓') : ''}
            </ClickableText>
          </Flex>
        )}
      </DashGrid>
