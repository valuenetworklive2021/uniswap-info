...
export const SUBGRAPH_HEALTH = gql`
  query health {
    indexingStatusForCurrentVersion(subgraphName: "valuenetworklive2021/valueswap-v2") {
      synced
      health
      chains {
        chainHeadBlock {
...
export const V1_DATA_QUERY = gql`
  query valueswap($date: Int!, $date2: Int!) {
    current: valueswap(id: "1") {
      totalVolumeUSD
      totalLiquidityUSD
      txCount
    }
    oneDay: valueswapHistoricalDatas(where: { timestamp_lt: $date }, first: 1, orderBy: timestamp, orderDirection: desc) {
      totalVolumeUSD
      totalLiquidityUSD
      txCount
    }
    twoDay: valueswapHistoricalDatas(
...
export const GLOBAL_CHART = gql`
  query valueswapDayDatas($startTime: Int!, $skip: Int!) {
    valueswapDayDatas(first: 1000, skip: $skip, where: { date_gt: $startTime }, orderBy: date, orderDirection: asc) {
      id
      date
      totalVolumeUSD
      dailyVolumeUSD
...
export const GLOBAL_DATA = (block) => {
  const queryString = ` query valueswapFactories {
      valueswapFactories(
