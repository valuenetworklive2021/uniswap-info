...
interface ReturnMetrics {
  ...
  valueswapReturn: number // netReturn - hodlReturn
  impLoss: number
  fees: number
}
...
export function getMetricsForPositionWindow(positionT0: Position, positionT1: Position): ReturnMetrics {
  ...
  const valueswap_return = difference_fees_usd + imp_loss_usd
  ...
  return {
    ...
    valueswapReturn: valueswap_return,
...
export async function getLPReturnsOnPair(user: string, pair, ethPrice: number, snapshots) {
  ...
  for (const index in snapshots) {
    ...
    valueswapReturn = valueswapReturn + results.valueswapReturn
