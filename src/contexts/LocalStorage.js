...
function init() {
  ...
  try {
    const parsed = JSON.parse(window.localStorage.getItem(VALUESWAP))
...
export function Updater() {
  ...
  useEffect(() => {
    window.localStorage.setItem(VALUESWAP, JSON.stringify({ ...state, [LAST_SAVED]: Math.floor(Date.now() / 1000) }))
  })
